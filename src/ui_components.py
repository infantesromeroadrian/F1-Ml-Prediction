...
class RaceProgressBarComponent(BaseComponent):
    ...
    def __init__(
        ...
        self._hover_event: dict | None = None
    ...
    def _draw_legend(self, window):
        ...
        for i, (color, symbol, label) in enumerate(legend_items):
            x = legend_x + (i * 45)
            arcade.Text(
                symbol, x, legend_y + 2, color, 10, bold=True, anchor_x="center", anchor_y="center"
            ).draw()
            arcade.Text(
                label, x, legend_y - 10, self.COLORS["text"], 8, anchor_x="center", anchor_y="top"
            ).draw()
...
class RaceControlsComponent(BaseComponent):
    ...
    def draw_hover_effect(
        self, button_name: str, x: float, y: float, radius_offset: int = 2, border_width: int = 4
    ):
    ...
    def _draw_speed_comp(self, x: float, y: float, speed: float):
        ...
        if "speed+" and "speed-" in self._control_textures:
            ...
            if not self._hide_speed_text:
                arcade.Text(
                    f"{speed:.1f}x", x, y - 5, arcade.color.WHITE, 14, anchor_x="center", bold=True
                ).draw()
