...
class F1RaceReplayWindow(arcade.Window):
    def __init__(
        ...
        self.visible_hud = (
            visible_hud  # If it displays HUD or not (leaderboard, controls, weather, etc)
        )
        self.ml_predictions = ml_predictions  # ML predictions DataFrame
    ...
    def on_draw(self):
        ...
        if self.bg_texture:
            arcade.draw_lrbt_rectangle_textured(
                left=0, right=self.width, bottom=0, top=self.height, texture=self.bg_texture
            )
        ...
        for code, pos in frame["drivers"].items():
            ...
            if (
                lap == 1
                and telemetry_dist < self._ref_total_length * 0.5
                and projected_m > self._ref_total_length * 0.5
            ):
        ...
        if self.ml_predictions is not None:
            self.leaderboard_comp.set_ml_predictions(self.ml_predictions)
