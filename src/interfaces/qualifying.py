...
class QualifyingReplay(arcade.Window):
    def __init__(
        ...
        self.race_controls_comp = RaceControlsComponent(center_x=self.width // 2 + 100, center_y=40)
        ...
        for res in self.data["results"]:
            if res["Q3"] is not None or res["Q2"] is not None or res["Q1"] is not None:
        ...
        if self.chart_active and self.loaded_telemetry:
            ...
            if frames:
                ...
                comparison_telemetry = (
                    self.data.get("telemetry", {})
                    .get(fastest_driver.get("code"))
                    .get("Q3")
                    .get("frames", [])
                    if self.show_comparison_telemetry
                    and fastest_driver
                    and (
                ...
                speed_bg = arcade.XYWH(
                    chart_left + chart_w * 0.5, speed_bottom + speed_h * 0.5, chart_w, speed_h
                )
                gear_bg = arcade.XYWH(
                    chart_left + chart_w * 0.5, gear_bottom + gear_h * 0.5, chart_w, gear_h
                )
                ctrl_bg = arcade.XYWH(
                    chart_left + chart_w * 0.5, ctrl_bottom + ctrl_h * 0.5, chart_w, ctrl_h
                )
                ...
                arcade.Text(
                    "Gear", chart_left + 10, gear_top + 10, arcade.color.ANTI_FLASH_WHITE, 14
                ).draw()
                arcade.Text(
                    "Throttle / Brake (%)",
                    chart_left + 10,
                ...
                all_dists = [
                    self._pick_telemetry_value(f.get("telemetry", {}), "rel_dist") for f in frames
                ]
                ...
                for dz in self.drs_zones:
                    ...
                    if current_dist >= zone_start:
                        ...
                        drs_zones_to_show.append({"zone_start": zone_start, "zone_end": shade_end})
                ...
                for dz in drs_zones_to_show:
                    ...
                    all_abs_dists = [
                        self._pick_telemetry_value(f.get("telemetry", {}), "dist") for f in frames
                    ]
                    ...
                    drs_rect = arcade.XYWH(
                        (x1pix + x2pix) * 0.5, speed_bottom + speed_h * 0.5, x2pix - x1pix, speed_h
                    )
                    ...
                    if comparison_telemetry and f_i < len(comparison_telemetry):
                        ...
                        if frame_comparison_telemetry is not None:
                            frame_comparison_telemetry = (
                                ...
                                if isinstance(frame_comparison_telemetry.get("telemetry", {}), dict)
                                else {}
                            )
                ...
                if (
                        ...
                        for dz in self.drs_zones_xy:
                            ...
                            try:
                                ...
                                interp_start_idx = int(
                                    (orig_start_idx / original_length) * interpolated_length
                                )
                                interp_end_idx = int(
                                    (orig_end_idx / original_length) * interpolated_length
                                )
                    ...
                    arcade.Text(
                        self.loaded_driver_code or "", sx + 10, sy + 4, arcade.color.WHITE, 12
                    ).draw()
                    if cur_gear is not None:
                        arcade.Text(
                            f"G:{int(cur_gear)}", sx + 10, sy - 10, arcade.color.LIGHT_GRAY, 12
                        ).draw()
    ...
    def load_driver_telemetry(self, driver_code: str, segment_name: str):
        ...
        if telemetry_store:
            driver_block = (
                telemetry_store.get(driver_code) if isinstance(telemetry_store, dict) else None
            )
            if driver_block:
                ...
                if seg and isinstance(seg, dict) and seg.get("frames"):
                    ...
                    self._speeds = (
                        np.array([float(s) for s in speeds if s is not None]) if speeds else None
                    )
        ...
        threading.Thread(
            target=self._bg_load_telemetry, args=(driver_code, segment_name), daemon=True
        ).start()
    ...
    def on_update(self, delta_time: float):
        ...
        if self._times is not None and len(self._times) > 0:
            ...
            self.frame_index = int(
                min(
                    self.n_frames - 1,
                    self.frame_index + int(round(delta_time * FPS * self.playback_speed)),
                )
            )
